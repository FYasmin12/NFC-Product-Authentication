...
if ($result->num_rows > 0) {
    ...
    if ($verification) {
        ...
        header("Location: index.php?status=verified_before&verified_at=" . urlencode($verification['verified_at']) . "&details=" . urlencode($product_details));
        exit();
    }
    ...
    if ($is_expired) {
        ...
        header("Location: index.php?status=verified_new_expired&details=" . urlencode($product_details));
        exit();
    } else {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: index.php?status=verified_new&details=" . urlencode($product_details));
            exit();
        } else {
